<?php
// Plugins tab — directory + per-plugin content.
// $tab, $activePlugin, $csrfToken available from index.php.

$pluginRegistry = [
    'schedule' => [
        'label'       => 'Course Schedule',
        'description' => 'Manage live and on-demand course dates, prices, and availability. Embed in any page with shortcodes.',
        'icon'        => '&#128197;',
    ],
];
?>
...
<?php if ($activePlugin === '' || !isset($pluginRegistry[$activePlugin])): ?>
    ...
    <div class="card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:14px;">
            <?php foreach ($pluginRegistry as $pid => $plugin): ?>
            <a href="?tab=plugins&plugin=<?= h($pid) ?>" style="text-decoration:none;color:inherit;">
                <div style="border:1px solid #e5e7eb;border-radius:8px;padding:18px 20px;background:#fff;transition:box-shadow .15s,border-color .15s;"
                    ...
                    <div style="font-size:1.6rem;margin-bottom:8px;"><?= $plugin['icon'] ?></div>
                    <div style="font-weight:600;font-size:1rem;color:#111827;margin-bottom:4px;"><?= h($plugin['label']) ?></div>
...
<?php else: ?>
    ...
    <div style="margin-bottom:14px;display:flex;align-items:center;gap:8px;font-size:.875rem;color:#6b7280;">
        ...
        <span>/</span>
        <span style="color:#111827;font-weight:500;"><?= h($pluginRegistry[$activePlugin]['label']) ?></span>
    </div>

    <?php require __DIR__ . '/schedule.php'; ?>
