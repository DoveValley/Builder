...
<div class="card" id="ms-upload">
    ...
    <p class="hint">Prepare the table in Excel or Google Sheets and <strong>Save As / Export &rarr; CSV</strong>. One row per site. Required columns: <code>domain, business, phone, email, city, state, SS</code>. Optional: <code>tel, address, zip, lat, lng, rating, review_count, analytics_id, logo, years_in_business, mission_statement, business_short</code> (a shorter brand form for page titles, e.g. "Bailey Restoration" — falls back to the full <code>business</code> name when left blank). Add FTP credentials (<code>ftp_host, ftp_user, ftp_pass</code>) to deploy &mdash; omit them and the row still builds.</p>
            ...
            ['section' => '1 &middot; Content', 'facet' => 'What the words say &mdash; the facet that actually costs rankings',
             'groups' => [
                ['key' => 'ai', 'label' => 'AI content', 'status' => 'live',
                 ...
                 'subs' => [
                    ['Research — per-city facts: industries, employers, market blurb, foundation/climate note, population, plus this niche\'s own chart figures (flood years, rainfall, etc.) and any custom research fields this niche declares directly &mdash; <em>checkpoint: skipped per FIELD, not per city — an already-researched city is only re-asked for whatever field is still missing or too short; a field the model can\'t find twice is left alone rather than re-billed forever</em>', 'auto'],
                    ...
                    ['Header info-popup disclosure — the "How Your Calls Are Handled" text shown when a visitor clicks the info icon next to the phone number, reworded per domain, one time; same legal meaning, only the phrasing changes &mdash; <em>checkpoint: stamped <code>_popup_reworded</code> after one attempt, success or failure, cached per domain so it\'s never re-billed on a routine rebuild</em>', 'control', 'ai.popup_reword'],
                    ...
                    ['Blog posts — 3 informational posts per domain, randomly picked (per-domain, stable across rebuilds) from a 25-topic pool per niche and written fresh by AI; a post only auto-publishes if it passes an automated quality check (word count, target-keyword cap, banned overclaim phrases) &mdash; otherwise it saves as a draft and gets logged instead of shipping &mdash; <em>checkpoint: picked topics and generated posts are cached per domain, so a rebuild tops up toward 3 instead of re-picking or re-billing</em>', 'control', 'ai.blog'],
