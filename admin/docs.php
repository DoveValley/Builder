...
<div id="sidebar">
    ...
    <nav id="infrastructure-nav" hidden>
        ...
        <a href="#infra-cloudpanel">CloudPanel deploy specifics</a>
...
<pre><code>Factory (this app)
  ...
  │  Push to Server (SFTP)  → deploy_site() in includes/multisite/deploy.php
  ▼
CloudPanel origin  (VPS, e.g. Hostinger)
  │  site files live at /home/{site-user}/htdocs/{domain}/
...
<p>Each layer is independent: the factory only cares about SFTP + files, CloudPanel only serves files, Cloudflare only proxies HTTP(S). Deploys never touch Cloudflare — they SFTP straight to the origin.</p>
...
<ol>
    ...
    <li><strong>SSL/TLS → set mode to Full (strict)</strong> — see the SSL section for why this is mandatory with CloudPanel.</li>
    <li><strong>Add a "Cache Everything" rule</strong> for static sites — see the Static + CDN section.</li>
</ol>
<div class="callout tip"><p><strong>CloudPanel real-visitor-IP:</strong> once traffic is proxied, every request arrives from a Cloudflare IP. Enable CloudPanel's Cloudflare / real-IP option so nginx logs, analytics, and <strong>fail2ban</strong> see the true client IP (from the <code>CF-Connecting-IP</code> header) instead of banning Cloudflare. This is an origin-side setting — it does <em>not</em> replace the account/zone/nameserver setup above.</p></div>
<a class="back-top" href="#infra-cloudflare">&uarr; top</a>
</section>

<section id="infra-ssl">
<h2>SSL/TLS &amp; certificates</h2>
<p>A self-signed cert (CloudPanel's default) makes browsers show "Not Secure." You need a trusted cert. Two routes:</p>
<table>
    <tr><th>Route</th><th>Best for</th><th>Notes</th></tr>
    <tr><td><strong>Let's Encrypt</strong> (via CloudPanel)</td><td>one-off / manual sites</td><td>Free, 90-day auto-renew. HTTP-01 challenge needs DNS resolving + port 80 reachable.</td></tr>
...
<div class="callout warn"><p><strong>Never use "Flexible" SSL with CloudPanel.</strong> CloudPanel force-redirects HTTP&rarr;HTTPS; Flexible makes Cloudflare talk HTTP to the origin &rarr; <strong>infinite redirect loop</strong>. This is the #1 CloudPanel + Cloudflare mistake.</p></div>
...
<section id="infra-cloudpanel">
<h2>CloudPanel deploy specifics</h2>
<p>The origins run <strong>CloudPanel</strong>. Hard-won lessons from bringing <code>p.q111.xyz</code> live:</p>
<ul>
    <li><strong>Deploy as the <em>Site User</em>, over SFTP on port 22</strong>, Remote Path <code>htdocs/{domain}</code> (home-relative to <code>/home/{site-user}</code>). This account has a clean, non-chrooted SFTP session.</li>
    <li><strong>Do NOT use CloudPanel's "FTP User".</strong> CloudPanel "FTP users" are <em>chrooted</em> SFTP accounts; the jail's ownership makes the SFTP subsystem <strong>exit with status 1</strong> immediately after auth &mdash; phpseclib reports <code>Expected NET_SFTP_VERSION</code>, the browser shows a generic "connection lost." Auth succeeds; the session dies. The site user avoids it.</li>
    <li><strong>Only port 22 (SSH/SFTP) is open</strong> in CloudPanel's firewall — no port 21, no FTP daemon. FTP is not an option there; use SFTP.</li>
    <li><strong>Repeated failed logins trip fail2ban</strong> &rarr; <code>ECONNREFUSED</code> on the <em>client</em> IP, and every retry resets the timer. The factory's own IP connects fine (successful auth isn't banned), so diagnose SFTP from the factory CLI, not a banned workstation.</li>
    <li><strong>After changing the user or path, the manifest is stale</strong> ("nothing to upload"). Run <strong>Force Push All</strong> once to seed the correct location; normal incremental pushes work after that.</li>
</ul>
<div class="callout tip"><p>Diagnose an SFTP handshake from the factory with <code>phpseclib3\Net\SFTP</code> + <code>define('NET_SSH2_LOGGING', 2)</code> then <code>$sftp-&gt;getLog()</code> — the SSH channel's <code>exit-status</code> reveals subsystem failures that the browser hides as "connection lost."</p></div>
<p>The Deploy tab itself (build, push, audit, force-push/delete over FTP or SFTP) is documented under the Admin reference; the transport is <code>deploy_site()</code> in <code>includes/multisite/deploy.php</code> (SFTP via vendored phpseclib, FTP via native <code>ftp_*</code>).</p>
<a class="back-top" href="#infra-cloudpanel">&uarr; top</a>
</section>

<section id="infra-automation">
<h2>Provisioning automation (planned — not built)</h2>
<p>For fleet scale, a script could provision a new site end-to-end across registrar + Cloudflare + CloudPanel, plugging into the params-CSV / <code>build_one</code> per-row model. API quality per system:</p>
<table>
    <tr><th>System</th><th>API</th><th>Notes</th></tr>
    <tr><td><strong>Cloudflare</strong></td><td>Excellent REST (v4)</td><td>Scoped tokens, official SDKs, Terraform provider. Add zone (returns NS), records, proxy, SSL mode, cache rules, purge, Origin CA certs.</td></tr>
    <tr><td><strong>Registrar (Namecheap)</strong></td><td>Workable, clunky</td><td>XML API; register + set custom NS (<code>domains.dns.setCustom</code>). Requires API enabled + IP allowlist + account threshold. Porkbun / Cloudflare Registrar are cleaner.</td></tr>
    <tr><td><strong>CloudPanel</strong></td><td>No REST API</td><td>Automate via <code>clpctl</code> CLI over SSH (<code>site:add:static</code>, site user, cert install). The weak link.</td></tr>
</table>
<p><strong>Pipeline (per site):</strong></p>
<ol>
    <li>Registrar API — register the domain.</li>
    <li>Cloudflare API — create the zone; capture assigned nameservers.</li>
    <li>Registrar API — set the domain's nameservers to Cloudflare's.</li>
    <li>SSH / <code>clpctl</code> — create the static site + site user + vhost (incl. <code>www</code>).</li>
    <li>Cloudflare API — create records (apex + www, proxied), set SSL Full (strict), add Cache-Everything rule.</li>
    <li>Install cert — see decision below.</li>
    <li>Factory — SFTP-deploy the static files (already automated).</li>
</ol>
<div class="callout tip"><p><strong>Key decision — certs at fleet scale:</strong> use <strong>Cloudflare Origin CA certificates</strong> (issued via the CF API, up to 15 years, no HTTP challenge, no rate limits) installed on the origin + SSL Full (strict) — <em>not</em> per-site Let's Encrypt. This sidesteps both the LE-while-proxied challenge failure and the LE rate limit. LE stays fine for one-off manual sites.</p></div>
<p><strong>Build notes:</strong> idempotent (check-exists before create), batch + backoff for rate limits, resumable, per-site state in the params CSV. Not yet built — a single-site proof-of-concept is the sensible first step.</p>
