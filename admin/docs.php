...
<div id="sidebar">
    ...
    <nav id="infrastructure-nav" hidden>
        <a class="nav-group" href="#infra-overview">Overview</a>
        <a href="#infra-stack">The stack, end to end</a>
        <a href="#infra-domains">Root domains only</a>
        <a href="#infra-dns">DNS: A records &amp; nameservers</a>
        <a href="#infra-cloudflare">Putting Cloudflare in front</a>
        <a href="#infra-ssl">SSL/TLS &amp; certificates</a>
        <a href="#infra-static-cdn">Static HTML + CDN caching</a>
        <a href="#infra-servers">Server strategy &amp; blast radius</a>
        <a href="#infra-footprint">Footprint / de-clustering</a>
        <a href="#infra-panel">HestiaCP deploy specifics</a>
...
<section id="tab-picdrop">
    ...
    <div class="callout tip"><p><strong>"Use current photo as reference" uses whichever side is currently active — Real or AI, not always Real.</strong> A slot can hold a Real photo and an AI photo side by side (see "Real vs. AI" below); the reference is always <code>$slot['value']</code> — the field's actual live value on the page right now — which is whichever side you last made active (via "Use this photo" or the "use Real"/"use AI" link). To generate from a specific starting photo, make that side active first.</p></div>
    ...
    <p><strong>The default subject line now uses the slot's alt text (fixed 2026-09-23).</strong> It previously named only the block's generic UI label ("Feature Split — Feature split"), which told the model nothing about what should actually be in frame — a real reference photo showing a technician beside a piece of equipment could come back with that equipment replaced by something unrelated, since nothing said it needed to survive. The default now prefers the slot's own alt text when one exists (often a real description like "Water damage restoration technician..."), with <code>{city}</code>/<code>{SS}</code>-style page shortcodes stripped first — an image model has no use for literal curly-brace text. The reference-edit path also gained an explicit "keep this close to the reference — same setting, same equipment, same vehicle if one is visible" instruction, matching what the batch pipeline (<code>includes/multisite/image_ai.php</code>) has always told the model. Verified with a real generation against a live reference photo: the equipment survived, only the requested change (a different face) applied.</p>
    <p><strong>Why results used to zoom in and crop equipment out (fixed 2026-09-23).</strong> The edit API only accepts 3 fixed output aspect ratios, and a real reference photo rarely matches one exactly — bucketing it into the nearest one forces the model to re-compose the shot to fit a differently-shaped canvas, which in practice reads as "zoomed in," and the result then gets centre-cropped to fit the actual slot on top of that. Rather than computationally preventing the mismatch (padding the canvas, cropping the result back apart — tried, reverted the same day as needless complexity for what a sentence in the prompt already fixes), the prompt itself now tells the model up front that a crop is coming: <em>"This photo will be cropped to fit its slot afterward, so leave margin around the subject and any equipment — do not compose it edge-to-edge."</em> Applies to both this tab's Generate button (<code>admin/picdrop_api.php</code>) and the automated batch pipeline (<code>includes/multisite/image_ai.php</code>).</p>
    <div class="callout warn"><p><strong>input_fidelity is NOT settable on this model.</strong> A same-day attempt also set <code>input_fidelity: high</code> on the edit call (a real OpenAI parameter for holding closer to a reference's structure) — reverted after a live call failed with <em>"The model 'gpt-image-2' does not support the 'input_fidelity' parameter."</em> That's a <code>gpt-image-1</code>-only parameter. Don't re-add it to <code>openai_images_build_edit_ch()</code>'s forwarded-fields list without confirming against whatever model <code>OPENAI_IMAGE_MODEL</code> is actually set to.</p></div>
    <p><strong>The public filename now carries a real local-SEO signal (added 2026-09-23).</strong> The saved photo used to be named from an opaque content hash alone (<code>ai_a1b2c3d4e5.webp</code>) — no city, no topic, nothing a search engine could read. Every OTHER image-differentiation scheme in this pipeline already bakes the city into the filename deliberately (see <a href="#spec-image-assign">4c</a>'s "a real local-SEO signal... not an arbitrary naming choice"), so an AI photo being the one anonymously-named file on an otherwise city-keyword-named site was itself an inconsistency, not a neutral choice. The public filename (what's actually written to <code>uploads/media/</code> and referenced on the page) is now built from the slot's own alt text — the same description now driving the prompt itself — plus the domain's city slug (<code>ms_slug_city()</code>) and a short hash purely to guarantee uniqueness (two slots can easily share near-identical alt text, unlike a hand-named upload). The <em>cache</em> filename (the internal lookup key in the domain's persisted-photos folder, never seen by a visitor) is unchanged — still the slot+prompt hash it always was, so prompt-edit cache invalidation keeps working exactly as before.</p>
...
<section id="ai-niche">
    ...
    <ul>
        ...
        <li><code>uses_research_fields</code> + <code>research_prompt</code> — whether the niche wants real per-city market data, and what to ask for (see <a href="#ai-research">Local market research</a>) — including <a href="#ai-regional-accuracy">niche-specific regional accuracy</a> (foundation type, climate risk, local pest species, water hardness, etc.).</li>
    </ul>
...
<section id="ai-research">
    ...
    <h3 id="ai-regional-accuracy">Regional accuracy — the highest-leverage use of this field</h3>
    <div class="callout" style="margin-bottom:14px;">
        <p style="margin:0;"><strong>Why this matters more than it looks like it should.</strong> A generic home-service AI writes about the United States as if it were one climate and one house type — mentioning basements in a slab-foundation region, or hurricanes in a landlocked one. That's not a small tone problem: to a real local reader, it's the single fastest tell that a page is templated, generic, thin content — worse for trust and worse for the doorway-page signal Google already watches for. <code>research_prompt</code>'s <code>local_note</code> field is the one mechanism in this codebase that fixes it, and it's <strong>per-niche</strong>, not per-city: the question changes by business type, but the same one-time-per-city research step answers it and every page for that city inherits the answer automatically — nothing to author per page, ever.</p>
    </div>
    <p>Each niche needs a genuinely different question — there is no one "regional accuracy" fact that fits every business type:</p>
    <ul>
        <li><strong>Water damage restoration</strong> — home foundation type (basement / crawl space / slab / pier-and-beam) <em>and</em> the dominant regional risk (hurricane/coastal flooding, snowmelt, heavy seasonal rain, winter freeze). Both vary independently by region and both change what a real local company would actually say.</li>
        <li><strong>Mold remediation</strong> — year-round ambient humidity (not storm events — mold cares about the average, not the emergency) and whether crawl spaces/basements are common. Humid Gulf Coast vs. arid Southwest is a bigger swing for this niche than foundation type alone.</li>
        <li><strong>Pest control</strong> — which pests are <em>actually present</em> (fire ants and Formosan termites are Southern/Gulf-specific, carpenter ants skew Northern, scorpions are Southwestern — "termites and ants" generically is an obvious tell to a local reader) and whether activity is year-round or seasonal.</li>
        <li><strong>Appliance repair</strong> — water hardness (hard-water regions cause specific, real damage to dishwashers/washers/ice makers/water heaters) and storm/power-surge frequency (ties to the same regional weather data other niches already use).</li>
    </ul>
    <p>Live on <code>water-site</code> / <code>mold-site</code> / <code>pest-template</code> / <code>appliance-site</code> as of 2026-08-31 — each master's <code>research_prompt</code> keeps items 1-3 and 5-6 of the shared default verbatim (industries, employers, market blurb, neighborhoods, population — already niche-agnostic and working) and swaps only item 4 (<code>local_note</code>) for its own version of the question above. <strong>Applies going forward automatically</strong> — any new city added to these four sites inherits the niche-specific question with zero extra setup; a batch/multisite rollout under one of these masters inherits it too, since it reads the master's own brief for every row. Cities already researched before this change keep their old, generic <code>local_note</code> until re-researched — this doesn't retroactively rewrite already-saved research.</p>
    <p><strong>Adding a 5th niche later:</strong> this is a one-time setup per niche, not per city or per site-clone — write that niche's version of "what's the one regional fact that actually matters here" into its own brief's <code>research_prompt</code> once, and every city ever added under it inherits it automatically from then on.</p>
...
<!-- ═══════════════════════ INFRASTRUCTURE ═════════════════════════════════ -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->
<div id="doc-infrastructure" hidden>
<h1>&#127760; Infrastructure</h1>
<p class="page-intro">How a built site actually gets from the factory to a live, secure URL — domains, DNS, Cloudflare, SSL, servers, and search footprint — plus the plan for automating it at scale. Context: the target is a network of <strong>static-HTML lead-gen sites</strong> (~400 sites across ~8 niches, ~50 per niche), each with unique content, phone, and its own root domain, generated by the factory and deployed to <strong>HestiaCP</strong> origins, all managed by the <strong>Infrastructure console</strong> (admin &rarr; &#128736; Infrastructure).</p>

<section id="infra-overview">
<h2>Overview</h2>
<p>The factory builds a site as flat static HTML and pushes the files to a web host. Everything on this page is about the layer <em>around</em> that: how the domain resolves, how HTTPS is served, how to keep origins fast and hidden, how many servers you actually need, and how to avoid looking like one obvious network to Google.</p>
<p>Two principles run through all of it:</p>
<ul>
    <li><strong>The signals search engines actually act on are content and intent</strong> — not IP addresses or DNS. Infra choices (IP/NS/data-center diversity) are the <em>weak</em> end of the footprint spectrum. Spend effort accordingly.</li>
    <li><strong>Static + a CDN in front makes most infra worries evaporate.</strong> When Cloudflare caches whole static sites at its edge, the origin is barely touched — so its location, count, and horsepower stop mattering for performance.</li>
</ul>
<a class="back-top" href="#infra-overview">&uarr; top</a>
</section>

<section id="infra-stack">
<h2>The stack, end to end</h2>
<p>For one site, the full path from build to visitor:</p>
<pre><code>Factory (this app)
  │  Generate Static Site  → output/{site_id}/  (flat HTML + assets)
  │  Push to Server (FTP/SFTP)  → deploy_site() in includes/multisite/deploy.php
  ▼
HestiaCP origin  (VPS — 20 boxes across 10 providers)
  │  site files live at /home/{account}/web/{domain}/public_html/
  │  the domain's FTP login is jailed to that directory, so it IS the login's home
  │  nginx serves them; origin holds a TLS cert
  ▼
Cloudflare  (proxy / CDN, optional but recommended)
  │  caches the static site at the edge worldwide
  │  terminates HTTPS for visitors; hides the origin IP
  ▼
Visitor  →  https://{domain}</code></pre>
<p>Each layer is independent: the factory only cares about FTP/SFTP + files, the origin only serves files, Cloudflare only proxies HTTP(S). Deploys never touch Cloudflare — they go straight to the origin.</p>
<a class="back-top" href="#infra-stack">&uarr; top</a>
</section>

<section id="infra-domains">
<h2>Root domains only — never subdomains</h2>
<p>Every site in the network gets its <strong>own separately-registered root domain</strong> (e.g. <code>dallaspestpros.com</code>). Do <strong>not</strong> run sites as subdomains of a shared domain (like <code>p.q111.xyz</code>, <code>m.q111.xyz</code>).</p>
<div class="callout warn"><p><strong>Why:</strong> subdomains of one registered domain are trivially clustered by Google as a single entity — a <em>stronger</em> "same network" signal than a shared server IP. Separate root domains are the single biggest de-clustering move.</p></div>
<p>Root domains also make everything downstream cleaner:</p>
<ul>
    <li>Each domain is its own <strong>Cloudflare zone</strong> — an isolated nameserver switch, no whole-domain entanglement.</li>
    <li><code>www.{domain}</code> becomes a real hostname worth putting on the cert (unlike <code>www.p.q111.xyz</code>, which is nonsense).</li>
    <li>One row per domain in the MultiSite params CSV — exactly the model the generator is built around.</li>
</ul>
<p>The existing <code>*.q111.xyz</code> subdomains (including the live <code>p.q111.xyz</code>) stay as test/reference deploys — don't tear them down — but all new sites are root-domain only.</p>
<a class="back-top" href="#infra-domains">&uarr; top</a>
</section>

<section id="infra-dns">
<h2>DNS: A records &amp; nameservers</h2>
<p>The one thing people get confused about: <strong>an A record always points to your origin server IP</strong>. You never point an A record "at Cloudflare." Cloudflare hooks in at the <strong>nameserver</strong> level instead.</p>
<h3>Without Cloudflare (registrar runs DNS)</h3>
<p>At the registrar (e.g. Namecheap → Advanced DNS): <code>@ → {server-IP}</code> and <code>www → {server-IP}</code>. That's live and doing the work.</p>
<h3>With Cloudflare (Cloudflare runs DNS)</h3>
<p>You change the domain's <strong>nameservers</strong> at the registrar to Cloudflare's pair. From that point:</p>
<ul>
    <li>The registrar's own A/DNS records go <strong>inert</strong> — nobody consults them. Leave or delete; doesn't matter.</li>
    <li>The live A record (still <code>{server-IP}</code>) now lives in the <strong>Cloudflare</strong> dashboard.</li>
</ul>
<p>Whether the public sees your real IP or Cloudflare's is the <strong>orange/grey cloud toggle</strong>, not the record's value:</p>
<table>
    <tr><th>Proxy toggle</th><th>A record value (in Cloudflare)</th><th>What the public resolves</th></tr>
    <tr><td>&#128992; Proxied (orange)</td><td><code>{server-IP}</code></td><td>a <strong>Cloudflare edge IP</strong> — origin hidden</td></tr>
    <tr><td>&#9898; DNS only (grey)</td><td><code>{server-IP}</code></td><td><code>{server-IP}</code> — origin exposed</td></tr>
</table>
<div class="callout"><p>On Cloudflare's free plan you can't proxy while keeping the registrar as your DNS — proxying requires Cloudflare to be the authoritative nameserver. It's all-or-nothing <em>per domain</em>: switch NS to Cloudflare, then manage records only there.</p></div>
<a class="back-top" href="#infra-dns">&uarr; top</a>
</section>

<section id="infra-cloudflare">
<h2>Putting Cloudflare in front</h2>
<p>Cloudflare works at the <strong>root-domain (zone)</strong> level. You add the whole domain and choose per-record what routes through the proxy. Step by step:</p>
<ol>
    <li><strong>Create a Cloudflare account</strong> (free plan is fine).</li>
    <li><strong>Add a site</strong> → enter the <strong>root domain</strong>. Cloudflare scans and imports existing DNS records — <strong>verify every record came across</strong> before continuing (a missing one goes dark after the NS switch).</li>
    <li><strong>Set proxy status</strong> per record: orange (proxied) on the apex + <code>www</code>; grey for anything you want left as a plain IP.</li>
    <li><strong>Copy the two Cloudflare nameservers</strong> it assigns.</li>
    <li><strong>At the registrar</strong> → Nameservers → Custom DNS → paste Cloudflare's two NS → save. Cloudflare shows <strong>"Active"</strong> when it detects the change (minutes to a couple hours).</li>
    <li><strong>SSL/TLS → set mode to Full</strong> for a static fleet (origin self-signed is fine); use <strong>Full (strict)</strong> only with a Cloudflare Origin CA cert on the origin. Never <em>Flexible</em> — see the SSL section.</li>
    <li><strong>Add a "Cache Everything" rule</strong> for static sites — see the Static + CDN section.</li>
</ol>
<div class="callout tip"><p><strong>Real-visitor-IP:</strong> once traffic is proxied, every request arrives from a Cloudflare IP. Configure real-IP handling on the origin (nginx <code>set_real_ip_from</code> with Cloudflare's ranges, reading <code>CF-Connecting-IP</code>) so logs, analytics, and <strong>fail2ban</strong> see the true client instead of banning Cloudflare. This is an origin-side setting — it does <em>not</em> replace the account/zone/nameserver setup above.</p></div>
<a class="back-top" href="#infra-cloudflare">&uarr; top</a>
</section>

<section id="infra-ssl">
<h2>SSL/TLS &amp; certificates</h2>
<p>A self-signed cert (what a box ships with) makes browsers show "Not Secure." You need a trusted cert. Two routes:</p>
<table>
    <tr><th>Route</th><th>Best for</th><th>Notes</th></tr>
    <tr><td><strong>Let's Encrypt</strong> (Hestia <code>v-add-letsencrypt-domain</code>)</td><td>one-off / manual sites</td><td>Free, 90-day auto-renew. HTTP-01 challenge needs DNS resolving + port 80 reachable.</td></tr>
    <tr><td><strong>Cloudflare Origin CA</strong> (via CF API)</td><td>fleet / automated provisioning</td><td>Up to 15-year cert, no HTTP challenge, no rate limits. Valid only between Cloudflare and origin. Install as a custom cert on the origin.</td></tr>
</table>
<p>With Cloudflare in front, set the Cloudflare SSL mode to <strong>Full (strict)</strong> — the origin keeps a valid cert (LE or Origin CA), and Cloudflare validates it end-to-end.</p>
<div class="callout warn"><p><strong>Never use "Flexible" SSL.</strong> Any origin that redirects HTTP&rarr;HTTPS — which is what you want it to do — combined with Flexible makes Cloudflare talk HTTP to that origin, get redirected to HTTPS, and come back round: an <strong>infinite redirect loop</strong>. This is the #1 panel + Cloudflare mistake, and it is not specific to any panel.</p></div>
<div class="callout warn"><p><strong>Installing a cert on Hestia takes a file, not a field.</strong> <code>v-add-web-domain-ssl</code> does not accept certificate <em>contents</em> — it reads <code>&lt;domain&gt;.crt</code>, <code>&lt;domain&gt;.key</code> and optionally <code>&lt;domain&gt;.ca</code> from a directory <strong>on the server</strong>, and the API has no file-upload verb. So the cert has to arrive by another transport first: the console uploads the three files over the site's own FTP account (which already points at the docroot), installs from there, then deletes them. <br><br><strong>Renewal is the case that bites.</strong> <code>v-add-web-domain-ssl</code> refuses to install over an existing cert (exit code 4), so the first install succeeds and every renewal fails. Replace with <code>v-change-web-domain-sslcert</code>, which never leaves the site without a certificate; deleting first and re-adding works but opens a second or two with no cert on a live site.</p></div>
<p><strong>Gotchas learned the hard way:</strong></p>
<ul>
    <li><strong>Let's Encrypt while proxied fails.</strong> An orange-clouded record makes Cloudflare intercept the HTTP-01 challenge. Either issue the cert before flipping to proxied, or (better at scale) use Cloudflare Origin CA and skip LE on the origin entirely.</li>
    <li><strong>LE rate limits at scale.</strong> ~300 new orders / 3 h per account — issuing hundreds of LE certs needs batching. Origin CA has no such limit.</li>
    <li><strong>The <code>www</code> challenge needs a vhost.</strong> Include <code>www.{domain}</code> in the cert only if nginx actually answers to it (add it to the site's server names first). On a subdomain site, <code>www</code> is pointless — issue the apex only.</li>
</ul>
<a class="back-top" href="#infra-ssl">&uarr; top</a>
</section>

<section id="infra-static-cdn">
<h2>Static HTML + CDN caching</h2>
<p>Because the sites are flat static HTML, Cloudflare can cache the <strong>entire</strong> site at its edge — and that changes the whole infrastructure calculus.</p>
<div class="callout warn"><p>Cloudflare caches static <em>assets</em> (CSS/JS/images) by default but <strong>does NOT cache HTML</strong> unless told to. For fully static sites, add <strong>Caching → Cache Rules → "Cache Everything"</strong> matching <code>{domain}/*</code> with a sensible edge TTL. Purge the cache on redeploy (or use a short TTL) so updates go live.</p></div>
<p>With Cache-Everything on:</p>
<ul>
    <li><strong>~100% edge hit rate</strong> — visitors are served from a Cloudflare node near them; the origin is touched only on a cache miss/revalidation.</li>
    <li><strong>Origin data-center location becomes irrelevant</strong> — no meaningful latency from where the origin lives.</li>
    <li><strong>One small VPS can back the whole network</strong> — it's essentially a rarely-hit file server. Capacity is a non-issue for 400 static sites.</li>
</ul>
<a class="back-top" href="#infra-static-cdn">&uarr; top</a>
</section>

<section id="infra-servers">
<h2>Server strategy &amp; blast radius</h2>
<p>Given static + edge caching, <strong>server count and location are never a performance decision</strong> — one modest VPS handles everything. The only reason to run more than one server is <strong>risk isolation and footprint</strong>.</p>
<ul>
    <li><strong>Split into a handful of servers</strong> (≈2–6) so one flagged IP, one outage, or one spam-report trace doesn't take down all 400. That's blast-radius containment — the one real benefit.</li>
    <li><strong>Skip global data-center scatter.</strong> Behind Cloudflare the origin DC is invisible; it's still all one hosting provider's <strong>ASN</strong> (trivially clustered); and static + edge means zero perf gain. Scattering 2–3 VPSs across 10 Hostinger data centers optimizes a layer nobody can see.</li>
    <li><strong>For real diversity, vary the <em>provider</em>, not the data center.</strong> Two or three <em>different</em> hosts break the ASN cluster far better than many locations of one host.</li>
    <li><strong>Region-match loosely.</strong> If sites target US markets, US/EU origins are marginally more natural than far-flung ones — but with Cloudflare it barely matters.</li>
</ul>
<a class="back-top" href="#infra-servers">&uarr; top</a>
</section>

<section id="infra-footprint">
<h2>Footprint / de-clustering</h2>
<p>"Doorway" is about <strong>content and intent</strong>, not DNS. A shared IP is normal and benign on its own. What lets Google (or a competitor's spam report) tie sites into one network is a <em>cluster</em> of signals — strongest to weakest:</p>
<table>
    <tr><th>#</th><th>Signal</th><th>Strength</th></tr>
    <tr><td>1</td><td>Shared analytics / GSC / AdSense IDs</td><td>Strong (deterministic)</td></tr>
    <tr><td>2</td><td>Duplicate / templated content</td><td>Strong</td></tr>
    <tr><td>3</td><td>Interlinking between the sites</td><td>Strong</td></tr>
    <tr><td>4</td><td>Same WHOIS / registrant / nameservers</td><td>Medium</td></tr>
    <tr><td>5</td><td>Shared server IP</td><td>Weak</td></tr>
</table>
<div class="callout warn"><p><strong>The real risk is 50 near-identical sites per niche.</strong> Post-2024, Google's scaled-content-abuse and site-reputation policies target exactly this. AI-templated pages with only city/phone swaps read as duplicates. This is where the network lives or dies — far more than any IP/DNS choice.</p></div>
<p><strong>Effort priority (do the top ones):</strong></p>
<ol>
    <li><strong>Genuine per-site content differentiation</strong> — vary templates/skins (theme presets + block skins), weave in real local specifics (neighborhoods, real data), not just token swaps.</li>
    <li><strong>Per-site analytics</strong> — the params CSV's <code>analytics_id</code> column; never share one property across sites.</li>
    <li><strong>No interlinking</strong> between the sites.</li>
    <li><strong>WHOIS privacy</strong>; spread domains across a couple of registrar accounts.</li>
    <li>IP / NS / data-center diversity — weak; do last, if at all.</li>
</ol>
<div class="callout"><p><strong>Cloudflare account caveat:</strong> all zones in one Cloudflare account tend to share the same assigned nameserver pair — a cleaner cluster signal than a shared IP. "One CF account per server" only gives NS diversity if you have multiple servers; a per-niche account split is an alternative. But since NS is a weak lever, don't over-invest.</p></div>
<a class="back-top" href="#infra-footprint">&uarr; top</a>
</section>

<section id="infra-panel">
<h2>HestiaCP deploy specifics</h2>
<p>The origins run <strong>HestiaCP 1.10.2 on Debian 13</strong> — 20 boxes across 10 providers. (Plesk was evaluated against it and lost on cost and capability in August 2026; CloudPanel before that had no REST API worth the name. Neither is in the estate or the codebase any more.) Provisioning, deploy, cert, and go-live are automated by the <strong>Infrastructure console</strong>; the mechanics below are what it drives.</p>
<ul>
    <li><strong>Deploy over FTP on port 21, passive</strong>, as the domain's own FTP account. The remote path is the <strong>login's own home</strong> — on Hestia the FTP user is jailed straight into the docroot, so there is no <code>/httpdocs</code> or <code>/public_html</code> below it. The exported params-CSV writes <code>/home/{ftp_user}</code>, and that is the whole path.</li>
    <li><strong>The FTP login is not the name you asked for.</strong> Hestia prefixes it with the owning account: ask for <code>site42</code> under account <code>fleet</code> and the real login is <code>fleet_site42</code>. Storing the argument instead of the returned login yields a credential that looks perfectly reasonable and fails every deploy with "login incorrect".</li>
    <li><strong>API</strong> at <code>https://{host}:8083/api/</code> — form-encoded POST, not REST. <code>access_key</code> and <code>secret_key</code> go as <strong>separate fields</strong> (not a combined header), plus <code>cmd</code> and positional <code>arg1</code>&hellip;<code>arg9</code>. The panel is self-signed out of the box, so this is the one outbound call in the console that skips certificate verification.</li>
    <li><strong>A site is three calls, and the third is not optional:</strong> <code>v-add-web-domain</code> (with the <code>www</code> alias), <code>v-add-web-domain-ftp</code> (per domain, not per account), then <strong><code>v-restart-web</code></strong>. Without the restart the domain is fully configured — it lists with the right document root, and an FTP upload lands in it — yet HTTP still serves Hestia's default "Success!" page, because nginx has not picked the vhost up. <code>v-rebuild-web-domains</code> does <em>not</em> fix it. Every step reports success; it is simply the wrong HTTP 200.</li>
    <li><strong>One owning account per box, one FTP account per domain.</strong> Sharing the owning account costs no credential isolation, because the FTP login that a deploy uses is per-domain either way.</li>
    <li><strong>Bulk provisioning should restart once at the end</strong>, not per site — restarting the web server 500 times is slow and is a small blip for every site already live on that box.</li>
    <li><strong>After a host/path change the manifest is stale</strong> ("nothing to upload"). Run <strong>Force Push All</strong> once to seed; incremental pushes work after.</li>
    <li><strong>Dotfiles:</strong> the FTP daemon may hide <code>.htaccess</code> from listings, so audits can report it "missing on server" even though it uploaded — a false negative. (Hestia serves with nginx, where <code>.htaccess</code> does nothing anyway.)</li>
</ul>
<div class="callout tip"><p>The Deploy tab (build, push, audit, force-push/delete over FTP or SFTP) is under the Admin reference; transport is <code>deploy_site()</code> in <code>includes/multisite/deploy.php</code>. The whole VPS&rarr;panel&rarr;Cloudflare&rarr;registrar lifecycle is managed in the Infrastructure console (<code>admin/infra/</code>), and every panel call goes through the one client in <code>admin/infra/lib/hestia.php</code>.</p></div>
<a class="back-top" href="#infra-panel">&uarr; top</a>
</section>

<section id="infra-automation">
<h2>Provisioning automation — BUILT (the Infrastructure console)</h2>
<p>What was once planned is now the <strong>Infrastructure console</strong> — an independent module at <code>admin/infra/</code> (admin &rarr; &#128736; Infrastructure). It provisions, deploys, and takes sites live end-to-end across registrar + Cloudflare + HestiaCP, tracked in a self-contained SQLite fleet state.</p>
<table>
    ...
    <tr><td><strong>Registrar</strong></td><td>7 wired, <strong>all buying</strong></td><td>NameSilo, Dynadot, Porkbun, Spaceship, Gandi, Namecheap, Cloudflare Registrar — every one can register a domain from the console, verified with a real purchase at each. Availability checking and NS switching on all but Cloudflare (which has neither, and needs neither). <strong>Namecheap cannot set auto-renew by API</strong> — see the console docs. An eighth, <strong>Cosmotown</strong>, is listed but has no API access yet. Managed from the Registers tab.</td></tr>
    <tr><td><strong>HestiaCP</strong></td><td>form-POST (<code>:8083</code>)</td><td><code>access_key</code> + <code>secret_key</code> as separate fields, <code>cmd</code> + <code>arg1..arg9</code>. Create web domain, per-domain FTP account, restart web, install cert, list/delete. Replaced Plesk (cost) and CloudPanel (no usable REST).</td></tr>
</table>
<p><strong>Pipeline (per domain — all in the console):</strong></p>
<ol>
    <li><strong>Register</strong> (optional auto-buy) via the registrar API.</li>
    <li><strong>HestiaCP</strong> — create the web domain + its FTP account, then restart the web server so nginx serves it.</li>
    <li><strong>Cloudflare</strong> — create zone; DNS apex+www proxied; SSL (Full); HSTS; capture nameservers.</li>
    <li><strong>Deploy</strong> — export creds &rarr; params CSV &rarr; MultiSite Build+Deploy uploads content to the FTP login's home.</li>
    <li><strong>Go-live</strong> — switch registrar nameservers to Cloudflare (scheduled ~20/day via cron); zone flips active.</li>
</ol>
<p><strong>Footprint:</strong> Bulk can round-robin each domain across servers / CF accounts / registrars. Fleet state, edit/remove, and a live dashboard are all in the console. Single &amp; bulk provisioning stream a live progress log.</p>
<div class="callout tip"><p><strong>Certs at fleet scale:</strong> a static fleet runs fine on Cloudflare <strong>Full</strong> (origin self-signed is fine, encrypts CF&harr;origin). Upgrade to <strong>Full (strict)</strong> only with a <strong>Cloudflare Origin CA cert</strong> on the origin (15-yr, no HTTP challenge, not in CT logs) — avoids per-site Let's Encrypt rate limits and the LE-while-proxied challenge failure.</p></div>
<a class="back-top" href="#infra-automation">&uarr; top</a>
</section>

</div><!-- /#doc-infrastructure -->
...
<p class="page-intro">The <strong>Infrastructure Console</strong> (admin &rarr; &#128736; Infrastructure, top nav next to MultiSite) manages the whole fleet's <em>infrastructure</em> — registering domains, creating sites on HestiaCP, wiring Cloudflare, and taking sites live — across many servers, Cloudflare accounts, and registrars. It's a self-contained module at <code>admin/infra/</code>, independent of the content factory. This page documents the console itself; the sister &#127760; <strong>Infrastructure</strong> tab covers the underlying network strategy (DNS, SSL, footprint).</p>

<section id="console-overview">
<h2>Overview</h2>
<p>The console automates the "plumbing" around a generated site: <strong>acquire the domain &rarr; provision (HestiaCP + Cloudflare) &rarr; deploy content &rarr; go live</strong>. It tracks every domain in a self-contained SQLite <strong>fleet state</strong>, talks to HestiaCP / Cloudflare / registrars over their APIs, and caches the expensive discovery so it stays fast at fleet scale.</p>
<ul>
...
<table>
    ...
    <tr><td><code>staged</code></td><td>The host on the server and/or the Cloudflare zone exist. Not public.</td></tr>
...
<table>
    ...
    <tr><td><strong>D.Buy</strong> (Domains)</td><td>The 9-column fleet table: <em>Domain · Ready to buy · Register · Buy date · Own · Cloudflare · VPS/host · State · Drift</em>. Covers the whole life of a domain from a name you typed to a live site. Load new names, check availability, schedule buys, and see everything reconciled against HestiaCP + Cloudflare with <strong>drift</strong> flags. Sortable on every column, server-side search + pagination (100/page). See <a href="#console-acquire">Acquiring domains</a>.</td></tr>
...
<table>
    ...
    <tr><td><code>hestia.json</code></td><td>HestiaCP servers</td><td><code>id</code>, <code>label</code>, <code>host</code>, <code>port</code> (8083), <code>access_key</code> + <code>secret_key</code>, <code>site_user</code> (the owning account), <code>default_ip</code>. Replaced <code>servers.json</code>, which was the registry of a panel that is gone.</td></tr>
    ...
    <tr><td><code>registrar.json</code></td><td>Registrar creds</td><td>keyed by name; <code>type</code> + per-registrar keys. <strong>Written by the Registers tab</strong> — you should not need to hand-edit this one.</td></tr>
...
<p>This is the stage <em>before</em> provisioning: getting from a list of names you'd like, to domains you actually own. It all happens on the <strong>D.Buy</strong> tab.</p>
...
<p>Columns 1 and 6&ndash;9 describe reality as discovered from HestiaCP and Cloudflare. Columns 2&ndash;5 are the acquisition stage. <strong>Every column sorts</strong>, and sorting applies to the whole set rather than the visible page.</p>
<table>
    ...
    <tr><td>7</td><td><strong>VPS / host</strong></td><td>—</td><td>Which server the site sits on, linking to that server's page.</td></tr>
...
<p>Both call one shared routine (<code>infra_provision_one()</code>), so single and bulk never drift. Per domain it: (optionally) registers &rarr; creates the web domain + its FTP account on HestiaCP (and restarts the web server so nginx serves it) &rarr; creates the Cloudflare zone &rarr; adds A records (apex + <code>www</code>) pointed at the VPS IP, proxied &rarr; sets SSL mode + HSTS &rarr; saves everything (incl. FTP creds) to fleet state. <strong>Idempotent</strong>: existing sites/zones are skipped/updated.</p>
<ul>
...
<h2>Registrars</h2>
<p>Managed from the <strong>Registers</strong> tab — no hand-editing of JSON required. Each card takes that registrar's API fields and has a read-only <strong>Test connection &amp; balance</strong> button. Credentials are written to <code>admin/infra/config/registrar.json</code> (gitignored, <code>0600</code>) and are <strong>never printed back into the page</strong>: a blank secret field means "keep the stored one", so you can re-save a card without re-typing a key.</p>
<p><strong>Eight registrars</strong> are listed: seven fully wired, plus Cosmotown, which has no API access yet. What each card advertises is a statement about that registrar's <em>API</em>, not about your account — <strong>Test</strong> is what tells you whether the credentials work and whether there are funds behind them.</p>
<p>Cards are titled <strong>Register 1&hellip;8</strong> in a deliberate order: the ones worth buying fleet domains at first, Namecheap late because its auto-renew cannot be set by API, and Cloudflare last because it cannot check availability and is a place to <em>hold</em> domains rather than shop at. That order lives in one place, <code>infra_registrar_types()</code>, and every registrar dropdown in the console follows it.</p>
<p>A card <strong>opens itself when something is wrong</strong> and stays shut otherwise, so the page is a list of one-line facts rather than a wall of forms. Its digest says how many domains are held there, how fast it can check names, whether auto-renew is settable, and whether there is a balance to read.</p>
<p>Auto-buy carries <strong>two separate marks</strong>, because collapsing them would let the page promise something the code cannot do:</p>
<ul>
<li><span class="badge b-ok">✓ auto-buy</span> — the registrar's API can register a domain <em>and</em> the purchase adapter is written here.</li>
<li><span class="badge b-warn">~ auto-buy</span> — the API can, but the adapter is not written yet. You can still assign and schedule domains to it (the plan is valid); the purchase lands with the buying step. <strong>All seven wired registrars can complete a purchase today</strong>, each verified with a real one, so nothing currently shows this mark — it is what the next registrar added will look like.</li>
<li><span class="badge b-mut">? auto-buy</span> — nothing is known yet, because there is no API access. Cosmotown is in this state: four <em>?</em> marks rather than four crosses, since a cross would claim the registrar cannot do these things when in fact nobody has told us.</li>
</ul>
<table>
    ...
    <tr><td>Porkbun</td><td><code>api_key</code>, <code>secret_api_key</code></td><td>✓ but ONE CHECK / 10 SEC</td><td>✓</td><td>✓</td><td>✗</td><td>"API Access" must be ON <em>per-domain</em>. Registration needs an integer <code>cost</code> equal to the price <strong>in cents</strong>, plus <code>agreeToTerms</code>. No balance endpoint. The rate limit makes it unusable for bulk availability — use NameSilo or Dynadot for lists.</td></tr>
    <tr><td>Spaceship</td><td><code>api_key</code>, <code>api_secret</code>, registrant contact</td><td>✓ whole list in one request, <strong>no price</strong> for standard names</td><td>✓</td><td>✓</td><td>✗</td><td>Namecheap's own subsidiary, and here for one reason: <strong>its API sets auto-renew, which its parent's cannot</strong>. Registration answers <strong>202 Accepted and completes asynchronously</strong>, so the domain is not registered the moment the call returns. Registration takes a contact <em>id</em>, so a contact is created once from the fields on the card and its id cached back into the config. No balance endpoint.</td></tr>
    <tr><td>Gandi</td><td><code>token</code></td><td>✓ one name per request, <strong>quotes a real price</strong></td><td>✓</td><td>✓</td><td>✗</td><td>One bearer token, no IP allowlist. The registrant contact is copied from a domain the account already holds, so no contact setup is needed before the first buy. ⚠ A Gandi token carries an explicit <strong>scope list</strong> and there is no way to test "can I create" without attempting a purchase — Test reports the scopes so the gap is visible rather than assumed. Registration answers 202 and completes asynchronously.</td></tr>
    ...
    <tr><td>Cosmotown</td><td>— none yet —</td><td>?</td><td>?</td><td>?</td><td>?</td><td><strong>API access not set up</strong>; asked support 2026-08-17, waiting. Sells .com near cost, which is the reason to have an account. Nothing is automated until they answer — but a domain can still be <em>assigned</em> to it, which records where the domain lives and makes go-live tell you to switch the nameservers by hand rather than silently skipping it.</td></tr>
    <tr><td>Cloudflare Registrar</td><td><code>account_id</code> + <strong>global key</strong></td><td>✗</td><td>✓</td><td>n/a</td><td>✗</td><td>Sells <strong>at cost</strong> — the cheapest place to hold a domain. Registers via <code>POST /registrar/<strong>registrations</strong></code> (not <code>/registrar/domains</code>, which 403s) and <strong>requires the global key</strong> — a scoped token gets "Authentication error". Needs a registrant contact; one is passed inline. No availability endpoint, so check the name elsewhere first. Billed to the card on file, so no balance to read. Nameservers n/a: a Cloudflare domain is always on Cloudflare nameservers, so go-live has nothing to switch.</td></tr>
</table>
<div class="callout"><p><strong>Cloudflare's credentials can be entered here, but usually are not.</strong> While nothing has been saved on this card, the console <em>falls back</em> to the Cloudflare account it already holds for zones, DNS and SSL (<code>admin/infra/config/cloudflare.json</code>) — the card marks that <strong>borrowed creds</strong> and shows the values that would really be used. Keeping the same token deliberately in two files would create two things to rotate and one to forget. Saving here pins different values and ends the fallback.</p>
<p><strong>Go-live is a no-op for a Cloudflare-registered domain.</strong> It is already on Cloudflare nameservers, so there is nothing to switch — the console reports that as success rather than as a failed NS change.</p></div>
<div class="callout"><p><strong>Spaceship and Gandi were dropped once and added back in August 2026.</strong> Spaceship had been rejected for being Namecheap-owned — a second account buying no real independence from that parent — and was reconsidered on a single merit that outweighs it: <strong>its API can set auto-renew, which Namecheap's cannot</strong>. Gandi came back because it quotes a real price on an availability check, which most of the others here do not. <strong>INWX stays dropped:</strong> its .com renews at $17.52 <em>plus</em> $5.03/yr for WHOIS privacy that every other registrar here includes free — roughly double the cheapest option.</p></div>
...
<p>Every purchase <strong>reads the auto-renew setting back from the registrar</strong> and records the answer, rather than reporting what was asked for. Column&nbsp;5 of the D.Buy table flags any owned domain that will not renew. Each behaviour below was found in practice, not read off a datasheet.</p>
...
<table>
    ...
    <tr><td>Spaceship</td><td class="ar-yes">✓ yes</td><td>set at registration, and changeable after</td></tr>
    <tr><td>Gandi</td><td class="ar-yes">✓ yes</td><td>separate call after registration</td></tr>
    <tr><td>Cosmotown</td><td>? unknown</td><td>no API access yet</td></tr>
...
<div class="ar-ok">
<h4>Spaceship &mdash; <span class="ar-yes">works</span></h4>
<p><code>PUT /domains/{domain}/autorenew</code> with <code>isEnabled</code>, and it can also be set at registration with <code>autoRenew</code>. This is the reason to hold domains here rather than at Namecheap, its own parent.</p>
</div>

<div class="ar-ok">
<h4>Gandi &mdash; <span class="ar-yes">works, as a second call</span></h4>
<p><code>PATCH /v5/domain/domains/{domain}/autorenew</code> with <code>enabled</code>. It is a <strong>separate call after registration</strong> — the create body has no auto-renew field — so a purchase that stops halfway leaves a domain that will not renew itself. A GET on that same path answers <strong>404 by design</strong>; only PATCH exists, which reads like a broken endpoint if you probe it the obvious way.</p>
</div>
...
<ol>
<li><strong>Register for more years up front.</strong> Multi-year at Namecheap costs about <strong>10&cent; per year</strong> more (1yr $11.28, 2&ndash;10yr $11.38/yr), so a 3- or 10-year term is near-free insurance &mdash; and a paid-up decade cannot fail, expire a card, or be switched off by accident. Set the <em>years</em> field on the New&nbsp;Site or Bulk form. <strong>This is no longer automatic:</strong> the console once forced a 3-year term on every Namecheap purchase, but across a few hundred domains that ties up two extra years of cost up front to defer the risk rather than remove it, so <strong>every registrar now buys a 1-year term</strong> and a Namecheap purchase warns instead. The one-click Buy buttons always buy one year.</li>
<li><strong>Prefer another registrar for fleet buying.</strong> NameSilo, Dynadot, Porkbun and Spaceship all set auto-renew correctly and verifiably. Spaceship is worth singling out: it is Namecheap's own subsidiary, so the limitation is Namecheap's API rather than anything structural.</li>
...
<p>Provisioning generates a unique FTP user per domain and stores it in fleet state. The <strong>Deploy</strong> view exports those as a <strong>params-CSV</strong> whose columns match the MultiSite params format exactly (<code>domain, ftp_protocol, ftp_host, ftp_port, ftp_user, ftp_pass, ftp_path, ftp_passive</code>, with the FTP login's own home as the path + FTP:21, passive). Merge those rows into a master site's params (MultiSite tab), then <strong>Build + Deploy</strong> uploads the generated content to the provisioned box.</p>
<div class="callout tip"><p>This is a one-way handoff (a CSV), which keeps the console independent of the factory. Only domains that actually have FTP creds (provisioned on a box) are exported. The path it writes is <code>/home/{ftp_user}</code> &mdash; <strong>not</strong> <code>/httpdocs</code> or <code>/public_html</code>, which are other panels' conventions and would upload every file into a folder nginx never reads.</p></div>
...
<p>Click a <strong>managed</strong> domain in the D.Buy list to open its manage page: edit <code>registrar</code> / <code>niche</code> / <code>status</code>, or run a destructive action. <strong>Every destructive action requires typing the domain to confirm.</strong></p>
<table>
    <tr><th>Action</th><th>Effect</th></tr>
    <tr><td>Delete Cloudflare zone</td><td>Removes the CF zone (DNS/SSL/HSTS). The host on the server + fleet record kept.</td></tr>
    <tr><td>Delete the host on the server</td><td>Removes the host + its files on the VPS. CF zone + fleet record kept.</td></tr>
    <tr><td>Remove from fleet (untrack)</td><td>Forgets the domain in the console. Leaves the host + Cloudflare intact.</td></tr>
...
<ul>
    ...
    <li><strong>Discovery cache</strong> — per-box site lists and Cloudflare zone lists are cached (~180s) in the same DB, so a warm page load makes <strong>zero API calls</strong>. <em>Discover / Refresh</em> (or <code>?refresh=1</code>) forces a live re-sweep; mutations invalidate it.</li>
    <li><strong>Pagination</strong> — the D.Buy view is server-paginated (100/page) with server-side search, so it stays light at thousands of domains.</li>
...
<ul>
    ...
    <li><strong>Secrets are never rendered back.</strong> The Registers form shows a saved key only as &ldquo;saved (<em>n</em> chars)&rdquo;; a blank secret field means <em>keep the stored value</em>, so a card can be re-saved without the key ever appearing in the HTML.</li>
    ...
    <li><strong>Redirects are not attacker-controlled</strong> — the D.Buy bulk endpoint validates its <code>back</code> parameter against its own query-string shape rather than trusting it, so it cannot be turned into an open redirect.</li>
    <li><strong>TLS</strong> — outbound API calls verify certificates by default; only the HestiaCP panel (<code>:8083</code>), which is self-signed out of the box, opts out explicitly.</li>
</ul>
...
<pre><code>admin/infra/
  index.php            router + dashboard
  bootstrap.php        auth gate, layout, nav, CSRF, flash, shared CSS/JS
  .htaccess            deny-by-default; only index.php and actions/*.php are reachable
  lib/
    http.php           curl JSON client (verify-by-default) + call counting
    hestia.php         THE panel client (create/list/delete web domain, FTP, cert, restart)
    hestia_fleet.php   the fleet: server registry, discovery, fleet-wide reads
    cloudflare.php     Cloudflare API (multi-account: zones/DNS/SSL/HSTS)
    registrar.php      8 registrar types + capability registry
    ...
    state.php          SQLite fleet state + counters + cache + D.Finder store
    cache.php          TTL discovery cache
    store.php          config-registry loaders
    cities.php         cities + niche planning
    keywords.php       Ahrefs / DataForSEO adapters
    serp.php           SERP readings (map pack · ads · directories)
    uptime.php         asks the site itself, not the panel
  ...
  config/  (gitignored)  hestia · cloudflare · registrar · domains · keywords  (+ .example)
  state/   (gitignored)  fleet.db</code></pre>
<div class="callout warn"><p><strong><code>lib/</code>, <code>views/</code>, <code>config/</code> and <code>state/</code> are denied to the web</strong> by <code>admin/infra/.htaccess</code>, deny-then-allow so a new secrets file is covered without anyone remembering. This was added after <code>config/hestia.json</code> and <code>state/fleet.db</code> were found answering <strong>200 on the public IP</strong>: mode <code>0600 www-data</code> is not protection when Apache <em>runs</em> as www-data, and <code>.gitignore</code> keeps a file out of the repo, not off the web.</p></div>
<p>Other parts of the panel may <code>require_once</code> a <code>lib/*.php</code> directly — they are self-contained. Do <strong>not</strong> require <code>bootstrap.php</code> from outside the console: it redirects when it cannot see a session, which turns a JSON endpoint into a 302.</p>
...
<table>
    ...
    <tr><td>Add a VPS</td><td><strong>Servers</strong> tab &rarr; add the box (host + access/secret key + IP), then Test from its own card. Writes <code>hestia.json</code>.</td></tr>
    <tr><td>Add a Cloudflare account</td><td>Add a row to <code>cloudflare.json</code> (account_id + token/global-key).</td></tr>
    <tr><td>Activate a registrar</td><td><strong>Registers</strong> tab &rarr; fill the fields &rarr; Save (it verifies immediately). Namecheap also needs this server's IP whitelisted in your Namecheap API settings.</td></tr>
    <tr><td>Check a registrar's funds</td><td><strong>Registers</strong> &rarr; Test. A valid key with no balance cannot buy &mdash; the card flags anything too low for a <code>.com</code>.</td></tr>
    <tr><td>Load domains to buy</td><td><strong>D.Buy</strong> &rarr; <em>+ Load new domains</em> &rarr; paste and/or upload a CSV. Safe to repeat; existing rows are never reset.</td></tr>
...
<div class="callout warn"><p><strong>A new host serves the default page until the web server restarts.</strong> <code>v-add-web-domain</code> reports success, the document root is right, and an FTP upload lands in it — but nginx has not picked the vhost up, so HTTP returns Hestia's "Success!" page with a perfectly healthy 200. The console calls <code>v-restart-web</code> for you; if you ever create a host by hand, that is the step to remember. When checking whether a site is really live, read the <code>&lt;title&gt;</code> — Hestia's placeholder <em>prints the domain name</em>, so "the page contains its own domain" is not a test.</p></div>
...
<section id="ms-visual-identity">
    ...
    <p><strong>Assignment (multisite build).</strong> The params <code>theme_preset</code> column names a preset (id or name) per row and always wins. <strong>Blank</strong> = a deterministic hash rotation off the domain (<code>ms_variant</code>, salt <code>'theme'</code>) across only the presets flagged <strong><code>in_rotation</code></strong> (falls back to all if none are flagged), so every site gets a distinct in-pool preset that's stable across rebuilds. Uncheck <em>In multisite rotation</em> on a preset to keep it for single-site use only.</p>

    <h3>Per-domain color jitter</h3>
    <p>Two domains landing on the same preset would otherwise share byte-identical theme colors. The <strong>jitter</strong> axis (the <code>visual.jitter</code> toggle on the batch panel) nudges each color a small, deterministic amount per domain — <code>ms_jitter_color()</code> (<code>includes/multisite/visual.php</code>) shifts hue ±10° (±4° for <code>accent_color</code>, which carries the brand and needs a tighter swing) and adjusts saturation/value, seeded from <code>domain + the color's own key</code> so the same domain always lands on the same shift and different domains land on different ones. <code>ms_apply_palette_jitter()</code> runs this over every <code>theme</code> color, then gates the result on WCAG AA contrast (4.5:1) — a jitter that would make text on that background less legible than the preset itself is reverted, never just "less bad than before."</p>
    <p><strong>Extended to hero_split's background (added 2026-09-23).</strong> <code>hs_bg_color</code> is a literal hex stored on the block itself, not a theme-mode value (<code>resolve_color()</code> only resolves <code>accent</code>/<code>header</code>/<code>footer</code>/<code>custom</code>) — so it sat outside the jitter above entirely, and every domain cloned from one master shared the exact same byte-identical hero background. <code>ms_jitter_hero_bg_blocks()</code> now jitters it too, same primitive, its own jitter key (<code>hero_bg_color</code>) so it doesn't move in lockstep with the theme's own colors. No separate contrast gate here — <code>hero_split</code> already picks white-vs-dark text from the background's actual rendered luminance (<code>includes/blocks.php</code>), not a fixed pairing, so it's self-correcting regardless of what the jitter produces. Runs on home + core (same <code>site.json</code> the palette jitter already touches) and on every already-generated landing page file, since those are separate files carrying their own literal copy of the template's color by the time this step runs.</p>

    <h3>The generated logo</h3>
    <p>The step applies the chosen Theme Preset (merges <code>preset.theme</code> → <code>data['theme']</code> and <code>preset.header</code> → <code>data['header']</code>), then separately picks a <strong>Logo Config</strong> from the site's <a href="#tab-genvisual">Logo Library</a> — an independent rotation pool, own CSV override (<code>logo_config</code>) and own hash rotate (<code>ms_variant</code>, salt <code>'logo'</code>), so a domain's logo arrangement and its color theme vary on separate axes. A Logo Config picks what goes on <strong>line 1</strong> and <strong>line 2</strong> — business name, city, or custom text — and which icon to weave in; <code>ms_resolve_logo_lines()</code> resolves that into concrete text, then <strong>generates a logo per site</strong> — a bug mark (the Logo Config's icon as an accent-colored silhouette on a dark rounded tile) to the left of a two-tone wordmark: line 1 in the accent color, line 2 in the dark brand color, left-justified (DejaVu Sans Bold). It's written to <code>uploads/</code> and set as <code>header.logo</code>. Colors always come from the applied Theme Preset (accent = <code>accent_color</code>; dark = <code>heading_color</code> → <code>footer_bg</code> → <code>header_bg</code> fallback) — never from the Logo Config, which only ever controls text and icon. A master with no Logo Library configured yet falls back to line 1 = business name, line 2 = city, no icon.</p>
...
<section id="ms-axes">
    ...
    <ol type="a">
        <li><span class="where perrow">Per-row</span>✅ Block-order / layout variations per domain — computed live at build time from the page's own current blocks (no per-page authoring step); "don't rotate top/bottom" pins that many leading/trailing blocks (1/1 default), the rest is shuffled and picked by domain hash. Covers the homepage and every landing page; configured right on the Generate Sites screen's Site structure variance card. <span class="pri must">Must</span> <span style="color:#64748b;">(see <a href="#ms-variation">Deterministic variation</a>)</span></li>
...
<section id="ms-specs">
    ...
    <div class="block-card-doc" id="spec-layout-skeletons">
        <h3>2a · Block-order / layout variations <span class="pri must">Must</span> <span class="where perrow" style="float:none;margin-left:6px;">Per-row</span></h3>
        <p class="bc-meta">✅ built (Home + Landing pages)</p>
        <p><strong>Description.</strong> Reorder each page's blocks per domain, so sites aren't structurally identical top to bottom.</p>
        <p><strong>Build (today, rebuilt 2026-09).</strong> No per-page authoring step — <code>layout_rotate_blocks()</code> (<code>includes/layout_variations.php</code>) computes the alternate ordering live from whatever blocks the page currently has: the first N and last M blocks (the "don't rotate top/bottom" pin counts, 1/1 default — hero pinned first, closing block pinned last) never move, and <code>ms_variant(domain, n, salt)</code> picks a shuffled ordering for what's between them (0 = natural). At build, <code>ms_differentiate_working_dir()</code> in <code>differentiate.php</code> applies this to the homepage's own blocks and to every landing page. On/off and the four pin counts (home top/bottom, landing top/bottom) are set right on the Generate Sites screen's "Site structure variance" card, not a separate per-page panel. Privacy/Terms/disclaimer/Contact Us and About Us are out of scope — see the doc comment on <code>ms_differentiate_working_dir()</code>.</p>
    </div>
    ...
    <div class="block-card-doc" id="spec-logo">
        ...
        <p><strong>Built.</strong> The <a href="#ms-visual-identity">visual-identity step</a> (<code>includes/multisite/visual.php</code>, run in <code>build_one.php</code> right after <code>ms_differentiate_working_dir()</code> and before the 4c prune) generates a logo per site: a <strong>bug mark</strong> (accent-colored icon on a dark rounded tile) to the left of a <strong>two-tone wordmark</strong> — line 1 in the accent color, line 2 in the dark brand color, left-justified (DejaVu Sans Bold). Written to <code>uploads/</code> and set as <code>header.logo</code>; colors come from the applied Theme Preset (accent = <code>accent_color</code>; dark = <code>heading_color</code> → <code>footer_bg</code> → <code>header_bg</code> fallback). This <strong>replaces the master's baked-in logo per site</strong> — the master's file goes unreferenced and the 4c prune removes it. Verified end-to-end. The params <code>logo</code> column stays a manual override, and the favicon (<a href="#spec-favicon">4a</a>) is derived from the same bug tile.</p>
        <p class="bc-meta">✅ UPDATED (2026-08-25) — line 1/line 2 text and icon are no longer derived from <code>{business}</code> by splitting on the first word. They now come from a <strong>Logo Config</strong> in the site's <a href="#tab-genvisual">Logo Library</a> (icon + a source per line: business name, city, or custom text) — a rotation pool fully independent of the Theme Preset that supplies the colors, with its own CSV override (<code>logo_config</code>) and hash rotate (salt <code>'logo'</code>, vs. the preset's <code>'theme'</code>). A master with no Logo Library yet falls back to line 1 = business, line 2 = city, no icon — the closest honest replacement for the old split, since that's the one arrangement that's exactly right whenever the business name doesn't already start with the city.</p>
    </div>
    ...
    <div class="block-card-doc" id="spec-image-assign">
        ...
        <p><strong>Hero text stamp on/off, per scope (added 2026-09-13).</strong> The batch panel's Images card has two dedicated switches — <code>images.stamp_home</code> and <code>images.stamp_landing</code> — that gate ONLY the keyword+city text overlay, independently for the homepage/core pages (<code>site.json</code>) vs. every landing page. Photo differentiation/renaming (the "varied" count) is never gated by these; only the text stamp itself turns off. Before this, hero stamping had no dedicated toggle at all — it was bundled inside the parent "Images" step checkbox alongside metadata stripping and photo variation, so turning it off meant losing those too.</p>
        ...
        <p><strong>Tuning &amp; locking the style.</strong> The <strong>Gen-Image</strong> tab (moved there from the Test Lab) previews the overlay on any image with live controls, then <em>Lock this style into the build</em> writes <strong>the active site's own</strong> <code>sites/{master}/multisite/hero_style.json</code> (position, colours, sizes + reference dims). The build scales the locked sizes to each hero, and the tab shares the exact render core, so the preview matches production. The same tab's <strong>photo-variation ranges</strong> save the same way, to <code>sites/{master}/multisite/image_variation.json</code>.</p>
        <div class="callout tip"><p><strong>Per-master, not global (fixed 2026-08-23).</strong> Both files resolve <strong>per-master first, repo-global <code>multisite/*.json</code> as fallback</strong>, and that order now lives in exactly one place — <code>ms_image_settings_locate()</code> / <code>ms_image_settings_read()</code> in <code>includes/multisite/image_overlay.php</code> — used by <code>build_one.php</code>, the single-site engine, the batch preflight and the Gen-Image tab alike. Before this, the tab read and wrote <em>only</em> the global copy while calling itself a per-master setting: a style locked on one master silently applied to every master, and was ignored outright by any master that had its own file. Saving now always writes the per-master copy; nothing writes the global one any more, though it is still read as a fallback for a master that has never locked a style. The tab states which of the three is in effect — this site's own file, the shared fallback, or the built-in defaults.</p></div>
        <p><strong>A separate system: AI-generated photos (Pic Drop).</strong> Everything above is the mechanical overlay/byte-perturbation pass — it never calls an AI model. A different, later pipeline (<code>includes/multisite/image_ai.php</code>, <code>ms_generate_ai_images_for_domain()</code>) generates genuinely new per-domain photos for slots approved in <a href="#tab-picdrop">Pic Drop</a>, by editing each domain's current photo with OpenAI's image-edit API rather than describing a scene from scratch — this keeps results close to what was actually approved instead of drifting. The edit instruction tells the model to keep the reference's subject, composition, and framing and make one small, deterministic, per-domain style change (lighting/mood, seeded from the domain) — real separate generations per domain, not a shared cache, which defeats exact-hash and perceptual-hash dedup checks.</p>
        <p><strong>Home hero composition varies on purpose (added 2026-09-23).</strong> The tight "same composition, same framing" instruction above defeats hash-based dedup, but a reverse image search (Google Images "search this image") compares visual structure, not bytes — so a composition-preserved photo can still read as the same image across domains. The single highest-visibility slot, the <strong>home hero</strong> (block type <code>hero</code> / <code>hero_split</code> / <code>hero_grid</code> on the homepage), is now allowed to vary framing, angle, and composition instead of holding them fixed. Every other AI photo slot keeps the original tight instruction unchanged — this is a scoped, deliberate exception, not a general loosening.</p>
        <p><strong>Four independent switches, and landing/service pages now in scope (added 2026-09-23).</strong> The batch panel's Images card used to have one combined <code>images.ai_photos</code> checkbox covering only the homepage and the site-wide <code>services_links</code> block — landing (per-service) pages had no AI-photo slots at all, even though Pic Drop already let you manually generate and confirm one there. It's now four independent switches — <code>images.ai_photos_hero_home</code>, <code>images.ai_photos_hero_landing</code>, <code>images.ai_photos_other_home</code>, <code>images.ai_photos_other_landing</code> — splitting both hero-vs-other AND home-vs-service-page, each showing its own slot count and per-run cost estimate in the pre-flight confirmation. <code>ms_image_ai_slot_category()</code> (<code>includes/multisite/image_ai.php</code>) is the one classification the batch panel's cost preview, <code>ms_batch_pending_summary()</code>, and the actual generation gate all call, so none of them can disagree about what a run will do. A hero slot on a service page gets the same "vary composition" treatment as the home hero (added 2026-09-23, above) — any hero is a page's highest-visibility photo, not just the home one.</p>
        <p><strong>How a landing slot is matched across domains.</strong> A clone's own generated page file is named after ITS OWN city (e.g. <code>tpl_water_damage_dallas-tx.json</code>), never the master's — so a landing-scope prompt can't be matched by the master's page filename the way home/global slots are matched by block id. Instead, capturing a landing slot in Pic Drop also records the page's <code>template_id</code> (read straight off the master's own generated page at that moment), and <code>ms_generate_ai_images_for_domain()</code> matches every domain's own <code>data/pages/*.json</code> files by that same field. A domain that never built a given page — Page Pool excluded it — is silently skipped for that slot; that's an expected, routine outcome, not a failure.</p>
    </div>
