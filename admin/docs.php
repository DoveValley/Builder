...
<pre><code>Factory (this app)
  ...
  ▼
Plesk origin  (VPS, e.g. Hostinger)
  │  site files live at /var/www/vhosts/{domain}/httpdocs/
  │  nginx/Apache serves them; origin holds a TLS cert
  ▼
...
<p>Each layer is independent: the factory only cares about FTP/SFTP + files, Plesk only serves files, Cloudflare only proxies HTTP(S). Deploys never touch Cloudflare — they go straight to the origin.</p>
...
<div class="callout tip"><p><strong>Real-visitor-IP:</strong> once traffic is proxied, every request arrives from a Cloudflare IP. Enable Plesk's Cloudflare / real-IP handling so nginx logs, analytics, and <strong>fail2ban</strong> see the true client IP (from the <code>CF-Connecting-IP</code> header) instead of banning Cloudflare. This is an origin-side setting — it does <em>not</em> replace the account/zone/nameserver setup above.</p></div>
<a class="back-top" href="#infra-cloudflare">&uarr; top</a>
</section>

<section id="infra-ssl">
<h2>SSL/TLS &amp; certificates</h2>
<p>A self-signed cert (Plesk's default) makes browsers show "Not Secure." You need a trusted cert. Two routes:</p>
<table>
    <tr><th>Route</th><th>Best for</th><th>Notes</th></tr>
    <tr><td><strong>Let's Encrypt</strong> (via Plesk)</td><td>one-off / manual sites</td><td>Free, 90-day auto-renew. HTTP-01 challenge needs DNS resolving + port 80 reachable.</td></tr>
...
<div class="callout warn"><p><strong>Never use "Flexible" SSL with Plesk.</strong> Plesk force-redirects HTTP&rarr;HTTPS; Flexible makes Cloudflare talk HTTP to the origin &rarr; <strong>infinite redirect loop</strong>. This is the #1 panel + Cloudflare mistake.</p></div>
...
<section id="infra-plesk">
<h2>Plesk deploy specifics</h2>
<p>The origins run <strong>Plesk Obsidian</strong>. (The original CloudPanel origin was decommissioned — the box was reinstalled to Plesk.) Provisioning, deploy, cert, and go-live are automated by the <strong>Infrastructure console</strong> (admin &rarr; &#128736; Infrastructure); the mechanics below are what it drives.</p>
<ul>
    <li><strong>Deploy over FTP on port 21</strong> as the domain's system (FTP) user, Remote Path <code>/httpdocs</code> (relative to the jailed home <code>/var/www/vhosts/{domain}</code>). Plesk's ProFTPd is open on 21; passive mode on. SFTP as the site user would need per-user shell access, so FTP:21 is the reliable transport.</li>
    <li><strong>REST API</strong> at <code>https://{host}:8443/api/v2</code> with an <code>X-API-Key</code> token (basic auth 401s on data endpoints). Creates sites (<code>POST /api/v2/domains</code>) and runs any <code>plesk bin</code> utility via <code>/api/v2/cli/{util}/call</code> (cert install, subscription remove, &hellip;).</li>
    <li><strong>fail2ban:</strong> whitelist the factory IP (<code>187.127.254.206</code>) in <code>/etc/fail2ban/jail.local</code> <code>ignoreip</code> — but keep it to <strong>one</strong> <code>ignoreip</code> line. A duplicate breaks Plesk's <code>f2bmng</code> and makes domain removal error out.</li>
    ...
    <li><strong>Dotfiles:</strong> ProFTPd hides <code>.htaccess</code> from listings, so audits may report it "missing on server" even though it uploaded — a false negative.</li>
</ul>
<div class="callout tip"><p>The Deploy tab (build, push, audit, force-push/delete over FTP or SFTP) is under the Admin reference; transport is <code>deploy_site()</code> in <code>includes/multisite/deploy.php</code>. The whole VPS&rarr;Plesk&rarr;Cloudflare&rarr;registrar lifecycle is now managed in the Infrastructure console (<code>admin/infra/</code>).</p></div>
<a class="back-top" href="#infra-plesk">&uarr; top</a>
</section>

<section id="infra-automation">
<h2>Provisioning automation — BUILT (the Infrastructure console)</h2>
<p>What was once planned is now the <strong>Infrastructure console</strong> — an independent module at <code>admin/infra/</code> (admin &rarr; &#128736; Infrastructure). It provisions, deploys, and takes sites live end-to-end across registrar + Cloudflare + Plesk, tracked in a self-contained SQLite fleet state.</p>
<table>
    ...
    <tr><td><strong>Registrar</strong></td><td>5 wired, <strong>all buying</strong></td><td>NameSilo, Namecheap, Porkbun, Dynadot, Cloudflare Registrar — every one can register a domain from the console, verified with a real purchase at each. Availability checking and NS switching on the first four (Cloudflare has neither, and needs neither). <strong>Namecheap cannot set auto-renew</strong> — see the console docs. Managed from the Registrars tab.</td></tr>
...
<ol>
    ...
    <li><strong>Plesk</strong> REST — create the site + FTP user.</li>
    <li><strong>Cloudflare</strong> — create zone; DNS apex+www proxied; SSL (Full); HSTS; capture nameservers.</li>
    <li><strong>Deploy</strong> — export creds &rarr; params CSV &rarr; MultiSite Build+Deploy uploads content to <code>/httpdocs</code>.</li>
...
<p><strong>Five registrars</strong> are supported. What each card advertises is a statement about that registrar's <em>API</em>, not about your account — <strong>Test</strong> is what tells you whether the credentials work and whether there are funds behind them.</p>
...
<li><span class="badge b-warn">~ auto-buy</span> — the API can, but the adapter is not written yet. You can still assign and schedule domains to it (the plan is valid); the purchase lands with the buying step. <strong>Only NameSilo can complete a purchase today.</strong></li>
</ul>
<table>
...
<div class="callout"><p><strong>Cloudflare's credentials are not entered on this page.</strong> Its card reuses the Cloudflare <em>account</em> the console already holds for zones, DNS and SSL (<code>admin/infra/config/cloudflare.json</code>). Keeping the same token in two files would only create two things to rotate and one to forget — so the card shows where the credentials come from, and offers Test but no Save.</p>
