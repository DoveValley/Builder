...
<table>
    ...
    <tr><td><strong>Registrar</strong></td><td>6 wired</td><td>NameSilo, Namecheap, Porkbun, Dynadot, INWX, Cloudflare Registrar. Availability + NS switching on the first five; <strong>only NameSilo can complete a purchase today</strong>, the rest are API-capable but their adapters come with the buying step. Cloudflare sells at cost but cannot register by API at all. Managed from the console's Registrars tab; anything else falls back to a manual NS switch.</td></tr>
...
<p><strong>Six registrars</strong> are supported. What each card advertises is a statement about that registrar's <em>API</em>, not about your account — <strong>Test</strong> is what tells you whether the credentials work and whether there are funds behind them.</p>
...
<table>
    ...
    <tr><td>INWX</td><td><code>username</code>, <code>password</code>, optional <code>totp_secret</code></td><td>✓ batched</td><td>~</td><td>✓</td><td>✓</td><td><strong>Session auth, not an API key</strong> — it logs in with a real account username and password, so use an API-only sub-account where you can. If 2FA is on, the shared secret is required. <strong>Untested here</strong>: no INWX account is configured, so these adapters have never run against the live API.</td></tr>
    <tr><td>Cloudflare Registrar</td><td><em>none — see below</em></td><td>✗</td><td>✗</td><td>n/a</td><td>✗</td><td>Sells <strong>at cost</strong>, so it is the cheapest place to hold a domain. But its API only lists and updates: <strong>registering has to happen in the Cloudflare dashboard</strong>, and there is no availability endpoint. Nameservers are marked n/a because a Cloudflare-registered domain is <em>always</em> on Cloudflare nameservers.</td></tr>
</table>
...
<div class="callout"><p><strong>Spaceship and Gandi were dropped.</strong> Spaceship is Namecheap-owned, so it costs a second account without buying any real nameserver independence — the whole point of using several registrars. Gandi was replaced by INWX.</p></div>
<div class="callout warn"><p><strong>Do not verify a registrar with an endpoint that does not check credentials.</strong> Porkbun's <code>/ping</code> answers SUCCESS for an empty key — using it as the test meant a typo'd credential still reported "API OK". The test calls an authenticated endpoint instead. Worth remembering when wiring the next registrar.</p></div>
