 ...
 *   city_niche  THE PLAN — one row per city you have picked for a niche, holding
 *               the score, the area code, the phone and the domain it maps to.
 *
 * A city is "selected" for a niche exactly when a city_niche row exists. Nothing
 * here is a receipt: a city can be re-scored, a domain re-pointed, a row dropped.
 * Ownership receipts stay on the domains table where they are sticky.
 */

const INFRA_CITY_COLS  = ['id', 'rank', 'city', 'state', 'ss', 'population', 'lat', 'lng', 'area_codes', 'ac_source'];
const INFRA_CN_COLS    = ['niche', 'city_id', 'ahrefs', 'score', 'score_src', 'area_code', 'phone', 'domain', 'note', 'created_at', 'updated_at'];
...
/** Add a city to a niche. Existing selections are never overwritten. */
function infra_cn_select(string $niche, string $cityId): bool
{
    $db = infra_cities_init();
    if (!infra_city_get($cityId)) return false;
    $st = $db->prepare('INSERT OR IGNORE INTO city_niche (niche,city_id,created_at,updated_at) VALUES (?,?,?,?)');
    $st->execute([$niche, $cityId, infra_now(), infra_now()]);
    return $st->rowCount() > 0;
}

function infra_cn_unselect(string $niche, string $cityId): void
{
    infra_cities_init()->prepare('DELETE FROM city_niche WHERE niche = ? AND city_id = ?')
        ->execute([$niche, $cityId]);
}
...
function infra_cn_counts(): array
{
    $rows = infra_cities_init()->query(
        'SELECT niche, COUNT(*) n, SUM(CASE WHEN domain <> "" THEN 1 ELSE 0 END) d
           FROM city_niche GROUP BY niche')->fetchAll(PDO::FETCH_ASSOC);
    $out = [];
