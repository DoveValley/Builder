            ...
            case 'zone':
                ...
                if (!$apex) {
                    ...
                    if ($want !== '' && $apex['ip'] !== $want) {
                        ...
                        // to flexible (CF↔origin plain HTTP, because that CF account has no
                        // origin_ca_token on file) — surface which one it actually is rather
                        // than let "resolves fine" stand in for "fully secure".
                        if ($acct) {
