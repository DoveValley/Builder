...
function allowed_block_types() {
    return [
        ...
        'trust_bar'       => 'Trust Bar (row of icon + label badges)',
...
function grouped_block_types(): array {
    return [
        ...
        'Social Proof' => [
            ...
            'trust_bar'    => 'Trust bar (icon + label badges)',
            'cards'        => 'Cards grid',
...
function block_descriptions(): array {
    return [
        ...
        'trust_bar'        => 'Full-width strip of icon + short-label trust badges',
...
function render_content_block($block, $pathPrefix = '') {
    ...
    switch ($type) {
        ...
        case 'cards':
            ...
            $badgeAccentMode  = $block['cards_badge_accent']       ?? $accentMode;
            $badgeAccentC     = $block['cards_badge_accent_custom']?? $accentCustom;
            ...
            $centeredClass = $isCentered ? ' cards-centered' : '';
            echo '<div class="cards-grid cards-grid-' . $cols . $centeredClass . '">';
            foreach ($items as $card) {
                ...
                echo '<div class="card-item' . ($isCentered ? ' card-item-centered' : '') . '"' . $cardBgStyle . '>';
                if ($cardIcon) {
                ...
                if ($cardBadge) {
                    ...
                    foreach (array_filter(array_map('trim', explode("\n", $cardBadge))) as $badge) {
                        echo '<span class="card-badge" style="border-color:'.h($badgeColor).';color:'.h($badgeColor).';">' . h($badge) . '</span>';
                    }
                    echo '</div>';
                }
        ...
        case 'trust_bar':
            ...
            $tbCheck    = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true" style="flex-shrink:0;"><path d="M20 6 9 17l-5-5" stroke="'.$tbCheckStroke.'" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';
            $tbRows = '';
            foreach ($tbItems as $tbItem) {
                ...
                $tbRows .= '<div style="display:flex;align-items:center;gap:10px;">'.$tbIconOut
                    .'<span style="font-weight:700;color:'.$tbTextColor.';font-size:var(--font-size-body);white-space:nowrap;">'
